...
class ImagecapturePostProcess():
    ...
    def get_schedule(self):
        ...
        for only_gs_case in self.model_input_data['Memory_onlyGsTW_list']:
            ...
            if self.model_obj.GP['ground_PAss_happens_'+s+'_'+str(n)].value() == 1 :
                st_gs = st_W
        ...
        image_capture_output = pd.DataFrame(  {"SatID":self.SatId_list,\
                        ...
                        "camera_memory_value_endofTW":self.camera_memory_value_list,\
                        "delta_camera_memory_value_in_this_TW":self.delta_camera_memory_value_list,\
                        "SSD_memory_value_endofTW":self.readout_memory_value_list,\
        ...
        image_capture_output['StripID'] = image_capture_output['encoded_strip_id'].map(self.model_input_data['stripid__encodedstripID'])
        image_capture_output['AoiID'] = image_capture_output['encoded_strip_id'].map(self.model_input_data['AOIid__encodedstripID'])
