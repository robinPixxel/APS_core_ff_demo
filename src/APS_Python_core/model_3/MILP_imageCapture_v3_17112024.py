...
class ImageCapturePlan():
            ...
            for s in self.data['imgery_sat_id_list']:
                for s1j1k1 in self.data['csjkSet__s'][s]:
                    ...
                    #     # self.prob += self.theth_o['overlap_dv_' + s1j1k1+'_'+s1j2k2 ] + self.theth_o['overlap_dv_' + s1j2k2+'_'+s1j1k1 ] <= 1,"overlap_cons_part4_"+'_'+s1j1k1+'_'+s1j2k2
                        ...
                        # self.prob += self.M  >= self.td_o['st_'+s1j2k2] +  self.x_o['oppr_'+s1j2k2 ]*(self.data['TW__csjk'][s1j2k2][1]\
                        #                                                                                             - self.data['TW__csjk'][s1j2k2][0])  \
                        #                                                                                             + self.setup_time\
                        #                                                                                             - self.td_o['st_'+s1j1k1]\
                        #                                                                                             + self.theth_o['overlap_dv_' + s1j1k1+'_'+s1j2k2 ]* self.M,"overlap_cons_part1_"+s1j1k1+'_'+s1j2k2
                        
                        # self.prob += self.theth_o['overlap_dv_' + s1j1k1+'_'+s1j2k2 ] + self.theth_o['overlap_dv_' + s1j2k2+'_'+s1j1k1 ] == 1 *(self.x_o['oppr_'+s1j1k1 ]+ self.x_o['oppr_'+s1j2k2 ]),"overlap_cons_part4_"+'_'+s1j1k1+'_'+s1j2k2
        ...
        if self.config['constraints']['power_constraints']:
            ...
            for only_gs_case in self.data['Power_GS_TW_list']:
                ...
                if eclipse_event == 1:
                    ...
                    self.prob += self.delta_power_value['delta_power_value'+s+'_'+str(n)] >= (idle_generate_eclipse - idle_consumption_eclipse)*(et_W-st_W) - self.M * self.GP['ground_PAss_happens_'+s+'_'+str(n)]
                    self.prob += self.delta_power_value['delta_power_value'+s+'_'+str(n)] <= (idle_generate_eclipse - idle_consumption_eclipse)*(et_W-st_W) + self.M * self.GP['ground_PAss_happens_'+s+'_'+str(n)]
                    ...
                    self.prob += self.delta_power_value['delta_power_value'+s+'_'+str(n)] >= idle_generate_sunlit * self.PtPG['Power_generation_'+s+'_'+str(n)] - idle_consumption_sunlit * (et_W - st_W) - self.M * self.GP['ground_PAss_happens_'+s+'_'+str(n)]
                    self.prob += self.delta_power_value['delta_power_value'+s+'_'+str(n)] <= idle_consumption_sunlit * self.PtPG['Power_generation_'+s+'_'+str(n)] - idle_consumption_sunlit * (et_W - st_W) + self.M * self.GP['ground_PAss_happens_'+s+'_'+str(n)]

                    self.prob += self.delta_power_value['delta_power_value'+s+'_'+str(n)] <= (downlinking_generate - downlinking_consumption) * self.Pgs['process_time_GP_'+s+'_'+str(g)+'_'+str(n)] \
                                                                                        + idle_generate_sunlit * self.Qtgs['Q_Power_generation_'+s+'_'+str(n)] \
                                                                                            - idle_consumption_sunlit * (et_W - st_W - self.Pgs['process_time_GP_'+s+'_'+str(g)+'_'+str(n)]) + self.M * (1-self.GP['ground_PAss_happens_'+s+'_'+str(n)])
    ...
    def create_objective(self):
        if self.config['objective']['GS_Pass_and_Imaging']:
            ...
            self.prob += (A + C)*10000000 + B*0.01 #(A + C)#(A + C)*1000 + B*0.01 #- D *10  # #A + C#C + B #A + C +B
        ...
        if self.config['objective']['total_readout_memory']:
            ...
            #self.prob += (A + C) >= int(self.data['GS_Pass_time_objective'])
