...
class GSpassSelection():
                ...
                for t in range(1,12*60*60):
                    ...
                    if self.data['temporal_data_flag'][s][t] != -1 : # GS PASS Possible
                        ...
                        for ind,item in enumerate(self.data['temporal_data_TW_list'][s][t]): #sorted TW with respect to starting time
                            ...
                            # self.prob += self.heat_binary['HB_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)]  <=  self.x['x_'+s+'_'+gs+'_'+str(twi)] * self.M# h=1 => x =1 ,h =0 => x=0,1
                            # #self.prob += self.heat_binary['HB_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)]*self.M  >=  self.x['x_'+s+'_'+gs+'_'+str(twi)] # h =1 =>x=0,1 ,h =0 ==> x =0

                            # self.prob +=  t  - self.t_d['st_'+s+'_'+gs+'_'+str(twi)] >= (1-self.heat_binary['HB_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)])*-1*self.M # h=1 ==> rc , h=0 ==> can be anything
                            # #self.prob +=  t  - self.t_d['st_'+s+'_'+gs+'_'+str(twi)] <= self.heat_binary['HB_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)]*1*self.M - 0.0001 # h=0 ==> r1c1 , h=1 ==>can be a nything

                            # self.prob +=  self.t_d['st_'+s+'_'+gs+'_'+str(twi)]+self.p['p_'+s+'_'+gs+'_'+str(twi)] -t  >= (1-self.heat_binary['HB_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)])*-1*self.M# h=1 ==> rc , h=0 ==> can be anything
                            # #self.prob +=  self.t_d['st_'+s+'_'+gs+'_'+str(twi)]+self.p['p_'+s+'_'+gs+'_'+str(twi)] -t  <= self.heat_binary['HB_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)] *1* self.M -0.0001# h=0 ==> r1c1 , h=1 ==>can be a nything
                            ...
                            # =========================================================================================================================================================================================================================
                            #self.prob += self.heat_binary['HB_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)] + self.steady_binary['CB_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)] <= 1

                            #self.prob += self.heat_binary['HB_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)] + self.steady_binary['CB_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)] + self.cool_binary['CB_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)] == 1
                            
                            # delta = self.heat_binary['HB_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)] * f(t-self.t_d['st_'+s+'_'+gs+'_'+str(twi)]) + \
                            #         self.steady_binary['steady_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)] * 0 + \
                            #         self.cool_binary['CB_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)] * f()
                            ...
                            self.v1 = {'v1_'+s+'_'+gs+'_'+str(twi)+'_'+str(t):LpVariable('v1_'+s+'_'+gs+'_'+str(twi)+'_'+str(t), cat='Binary' ) }
                            self.v2 = {'v2_'+s+'_'+gs+'_'+str(twi)+'_'+str(t):LpVariable('v2_'+s+'_'+gs+'_'+str(twi)+'_'+str(t), cat='Binary' ) }
                            
                            self.prob +=  t - self.t_d['st_'+s+'_'+gs+'_'+str(twi)] - 5 <=\
                            (1- self.v1['v1_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)]) * self.M #=> v1 = 1 , t - t_d <= 5
                            
                            self.prob += t - self.t_d['st_'+s+'_'+gs+'_'+str(twi)] >=\
                            5.00001 - (1-self.v2['v2_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)])  * self.M #=> v2 = 1 , t - t_d >5 
                            #self.prob +=  t - self.t_d['st_'+s+'_'+gs+'_'+str(twi)] >=  5.00001 - (1-v2)  * M  => v2 = 1 , t - t_d >5 
                            
                            # self.prob += self.v1['v1_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)] * (t-self.t_d['st_'+s+'_'+gs+'_'+str(twi)]) +\
                            # self.v2['v2_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)]* 0.5* (t-self.t_d['st_'+s+'_'+gs+'_'+str(twi)])                          #  v1 * f1(t-d) + v2 * f2(t-t_d) - v3 <= (1-hb)  * M
                            #  v1 * f1(t-d) + v2 * f2(t-t_d) - v3 >= (1-hb) * M  * -1
                            # v1 + v2 == hb  
                            # v3 <= hb * self.M
                            # delta  = v3 (for heating)
                            delta = self.v1['v1_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)] * (2) + self.v2['v2_'+s+'_'+gs+'_'+str(twi)+'_'+str(t)]* 2.5
    ...
    def solve_model(self):
        print("start_solving")
        status = self.prob.solve(PULP_CBC_CMD(msg=False,timeLimit = 90 ))
        print("==================")
