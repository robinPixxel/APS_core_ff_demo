...
def select_img_opprtunity(image_opportunity_df,gs_pass_result_df,eclipse_df_dict,config):
    ...
    data['readout_memory_capacity__s'] = {s:v for s,v in data['camera_memory_capacity__s'].items() }
...
def get_input_files(config):
    ...
    assured_capture_df = pd.DataFrame({'encoded_stripId':total_capture_list,'assured_task':[1,1]+[0]*(no_of_list-2)})
...
if __name__ == "__main__":
    ...
    print(img_capture_result,APS_success_metric_df,downlink_result)
